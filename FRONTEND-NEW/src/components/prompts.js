...
const Prompts = () => {
  ...
  const [projectId, setProjectId] = useState(() => {
    ...
    return stateId ? String(stateId) : storedProjectId;
  });
  const [projectName] = useState(() => {
    const stateName = location.state?.projectName;
    return stateName || storedProjectName;
  });
  ...
  useEffect(() => {
    if (location.state?.projectId) {
      ...
      setActiveProject({ id: nextId, name: location.state?.projectName || null });
    }
  }, [location.state, setActiveProject]);
  ...
  const activateProjectByName = useCallback(async (name) => {
    ...
    try {
      ...
      notify.error(error.message || "Failed to activate project.");
      return null;
    }
  }, [apiBase, token]);
    ...
    const fetchPromises = promptFiles.map(name =>
      fetch(`${apiBase}/projects/${targetProjectId}/files/content?path=prompts/${name}`, { headers })
        ...
        .catch(error => {
          ...
          notify.error(`Error loading prompt ${name}: ${error.message}`);
  ...
  useEffect(() => {
    if (!token) {
      ...
      notify.error("No active project or user session. Please start from the dashboard.");
      navigate("/");
      return;
    }
    ...
    const fetchPrompts = async () => {
      ...
      try {
        ...
        if (!activeId) {
          if (!projectName) {
            notify.error("No active project. Please start from the dashboard.");
            navigate("/");
            return;
          }
        ...
        if (activeId && activeId !== projectId) {
          ...
          setActiveProject({ id: activeId, name: projectName || null });
        }
      } catch (error) {
  ...
  const handleSave = async (index) => {
    ...
    if (!targetProjectId) {
      notify.error("No active project. Please reopen the prompts editor.");
      return;
    }
    ...
    try {
      ...
      notify.success(`Prompt '${promptToSave.name}' saved successfully!`);
