...
def enter_originating_lead(page, value):
    page.smartAI('detail_originating_lead_textbox_originating_lead_field_c230994c').fill(value)
...
def enter_originating_lead(page, value):
    # Prefer SmartAI locator
    try:
        loc = page.smartAI('detail_originating_lead_textbox_originating_lead_field_c230994c')
        try: loc.focus(timeout=1000)
        except Exception: pass
        try: loc.fill("")
        except Exception: pass
    ...
    _ph = "Originating Lead"
    _lbl = "Originating Lead"
    if _ph:
        try:
            loc = page.get_by_placeholder(_ph).first
            loc.click()
    ...
    try:
        ...
        raise AssertionError(f"Unable to fill input for '{_lbl or _ph or 'detail_originating_lead_textbox_originating_lead_field_c230994c'}': {e}")

def assert_enter_originating_lead(page, expected: str, timeout: int = 6000):
    exp = str(expected)
    # 1) Prefer SmartAI target
    try:
        locator = page.smartAI('detail_originating_lead_textbox_originating_lead_field_c230994c')
        try:
            expect(locator).to_have_value(exp, timeout=timeout)
            return
        except Exception:
    ...
    try:
        ...
        try:
            ...
            if _values_match(actual, exp):
                return
            raise AssertionError(f"Assertion failed for '{lbl or ph or 'detail_originating_lead_textbox_originating_lead_field_c230994c'}' expecting '{exp}' (actual '{actual}'): {e}")
    except Exception as e:
        raise AssertionError(f"Assertion failed for '{lbl or ph or 'detail_originating_lead_textbox_originating_lead_field_c230994c'}' expecting '{exp}': {e}")
