...
def get_project_history_path(project_dir: Optional[str] = None, src_dir: Optional[str] = None) -> Path:
    """
    ...
    """
    if src_dir:
        return Path(src_dir) / "history.json"
    if project_dir:
        return Path(project_dir) / "generated_runs" / "src" / "history.json"
    raise ValueError("project_dir or src_dir is required to resolve history.json")


class MetricsStore:
    def __init__(self, project_id: int, history_path: Optional[Path] = None):
        if project_id is None:
            raise ValueError("project_id is required for MetricsStore")
        self.project_id = project_id
        self.path = history_path
        if self.path is None:
            ctx = get_project_context(required=True)
            self.path = get_project_history_path(
                project_dir=str(ctx.project_dir),
                src_dir=str(ctx.generated_src_dir),
            )
