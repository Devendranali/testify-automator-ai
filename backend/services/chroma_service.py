...
def upsert_text_record(chromaPath , record: dict):
    ...
    where_clause = {"label_text": metadata.get("label_text")}
    if pid is not None:
        where_clause = {"$and": [where_clause, {"project_id": pid}]}
    ...
    try:
        ...
        embedding_value = get_sentence_transformer_embedding_function()([text_to_embed])[0]
        collection.upsert(
            ids=[record["id"]],
...
def upsert_element_record(record: dict):
    ...
    try:
        embedding_function = get_sentence_transformer_embedding_function()
        collection = get_collection(_require_chroma_path(), "element_metadata", embedding_function)
