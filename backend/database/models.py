...
class OrganizationMember(Base):
    ...
    @classmethod
    def ensure_member(cls, db: Session, user_id: int, organization_id: int, role: str = "member") -> "OrganizationMember":
        existing = (
            db.query(cls)
            .filter(cls.user_id == user_id, cls.organization_id == organization_id)
            .first()
        )
        if existing:
            return existing
        record = cls(user_id=user_id, organization_id=organization_id, role=role)
        db.add(record)
        db.flush()
        return record

    @classmethod
    def user_org_ids(cls, db: Session, user_id: int) -> set[int]:
        rows = (
            db.query(cls.organization_id)
...
class TestCaseMetadata(Base):
    ...
    __table_args__ = (
        ...
        UniqueConstraint("project_id", "display_name", name="uq_test_case_metadata_project_display_name"),
        Index("ix_test_case_metadata_project_id", "project_id"),
        Index("ix_test_case_metadata_script_path", "script_path"),
    )

    id = Column(Integer, primary_key=True, index=True)
    project_id = Column(Integer, ForeignKey("projects.id", ondelete="CASCADE"), nullable=False)
    case_uuid = Column(String(64), nullable=False, unique=True, index=True)
    test_name = Column(String(1024), nullable=False)
    display_name = Column(String(255), nullable=False)
    user_story = Column(Text(), nullable=False, default="")
    auto_testcase = Column(Text(), nullable=False, default="")
    steps_json = Column(JSON, nullable=True)
    test_type = Column(String(32), nullable=True)
    markers = Column(JSON, nullable=False, default=list)
    tags = Column(JSON, nullable=False, default=list)
    priority = Column(String(50), nullable=True)
    script_path = Column(String(1024), nullable=True)
    runner_script_path = Column(String(1024), nullable=True)
    created_at = Column(DateTime(timezone=True), server_default=func.now(), nullable=False)
    updated_at = Column(
        DateTime(timezone=True),
        server_default=func.now(),
    ...
    def to_dict(self) -> dict:
        ...
        try:
            ...
            if "steps_json" not in state.unloaded:
                steps_value = self.steps_json
        except Exception:
            try:
                steps_value = self.steps_json
            except Exception:
        ...
        return {
            ...
            "auto_testcase": self.auto_testcase,
            "steps": steps_value,
