...
def _parse_story_block(story: str) -> Tuple[str, List[Dict[str, str]]]:
    ...
    for raw in story.splitlines():
        ...
        m = PATTERNS["click_button"].search(line)
        if m:
            steps.append({"type": "click_button", "name": m.group(1)}); continue
        m = PATTERNS["click_link"].search(line)
        if m:
            steps.append({"type": "click_link", "name": m.group(1)}); continue
        m = PATTERNS["click_plain"].search(line)
        if m:
            steps.append({"type": "click_plain", "name": m.group(1)}); continue

        m = PATTERNS["enter_field_q"].search(line) or PATTERNS["enter_field_nq"].search(line)
        if m:
            steps.append({"type": "enter_field", "value": m.group(1), "label": m.group(2)}); continue

        m = PATTERNS["select_q"].search(line) or PATTERNS["select_nq"].search(line)
        if m:
            steps.append({"type": "select_field", "value": m.group(1), "label": m.group(2)}); continue
...
# =============================================================================
# UI script (SmartAI-first, medium typing, no hardcoded values)
# =============================================================================
_UI_HEADER = '''# Auto-generated UI runner
...
def _generate_ui_script_for_test_file(test_path: Path) -> str:
    # page imports (except __init__)
    page_imports: List[str] = []
    for py in sorted(PAGES_DIR.glob("*.py")):
        if py.name == "__init__.py":
            continue
        page_imports.append(f"from pages.{py.stem} import *")

    # read tests
    test_src = test_path.read_text(encoding="utf-8")

    # extract test_* bodies
...
async def generate_from_user_story(
    ...
    stub_path = _write_auto_stubs(needed if len(needed) else set())

    test_idx = _next_index()
    test_file = TESTS_DIR / f"test_{test_idx}.py"

    # stubs first (so real page methods can override)
    import_lines = ["from pages.auto_stubs import *"]
    for module_name in sorted(method_map_full.keys()):
        import_lines.append(f"from pages.{module_name} import *")
    ...
    return {
        ...
        "log_file": str(log_file),
        "stubs_file": stub_path,
        "missing_methods_count": len(needed),
    }
