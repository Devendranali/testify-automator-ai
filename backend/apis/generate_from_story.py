...
# Switch to method-specific assertion injection: assert_<method_name>(page, value)
...
def generate_test_code_from_methods(user_story: str, method_map: dict, page_names: List[str], site_url: str) -> str:
    ...
    for methods in method_map.values():
        for method in methods:
            name = method.split("(")[0].replace("def ", "").strip()  # just the function name
...
def get_inferred_pages(user_story: str, method_map_full: dict, openai_client) -> List[str]:
...
"""
    ...
    try:
        ...
        # Fallback: all pages in current order
...
async def generate_from_user_story(
    ...
    # Snapshot current chroma metadata
