...
def get_metrics_store(
    project_id: int = Query(..., description="Project ID to scope metrics to"),
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
) -> MetricsStore:
    project = get_project_by_projectId(db, project_id, current_user)
    project_paths = _ensure_project_structure(project)
    history_path = Path(project_paths["src_dir"]) / "history.json"
    return MetricsStore(project_id=project.id, history_path=history_path)
