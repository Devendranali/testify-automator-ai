...
async def _rich_extract_dom_metadata(fr: Union[Page, Frame]) -> List[Dict[str, Any]]:
    ...
    (() => {
      ...
      const pick=el=>{
        ...
        const r = {
          tag, role, id:el.id||"", name:el.getAttribute&&el.getAttribute("name")||"", type:el.getAttribute&&el.getAttribute("type")||"",
          text:textOf(el), aria_label:el.getAttribute&&el.getAttribute("aria-label")||"", placeholder,
          label_text:accessibleName(el), title:el.getAttribute&&el.getAttribute("title")||"",
          data_testid:el.getAttribute&&el.getAttribute("data-testid")||"", visible:true,
      ...
      const pickText=el=>{
        ...
        const tag=(el.tagName||"").toLowerCase(); if(!/^(h1|h2|h3|h4|h5|h6|label|legend|span|strong|em|p|div)$/.test(tag)) return;
        ...
        push({ tag, role:el.getAttribute&&(el.getAttribute("role")||""), id:el.id||"", name:el.getAttribute&&el.getAttribute("name")||"", type:"",
          ...
          data_testid:el.getAttribute&&el.getAttribute("data-testid")||"", visible:true, bbox:{x:rect.x,y:rect.y,width:rect.width,height:rect.height}});
      };
      const visit=root=>{
        if(!root) return;
...
async def _run_enrichment_for(page_name: str) -> Dict[str, Any]:
    ...
    for fr in frames:
        block = await extract_dom_metadata(fr, CURRENT_PAGE_NAME) or []
        try: block = list(block)
        except Exception: block = [block]
        if _assess_dom_quality(block):
