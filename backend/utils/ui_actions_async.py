...
from playwright.async_api import BrowserContext, Dialog, Download, FileChooser, Frame, FrameLocator, Locator, Page, Request, Response, expect


async def safe_wait_ready(page: Page, wait_until: str = "domcontentloaded", timeout: int = 60000) -> None:
    await page.wait_for_load_state(wait_until, timeout=timeout)

async def safe_wait_url(page: Page, url_or_pattern, timeout: int = 60000) -> None:
    await expect(page).to_have_url(url_or_pattern, timeout=timeout)

async def safe_wait_load_state(page: Page, state: str = "domcontentloaded", timeout: int = 60000) -> None:
...
async def safe_select(
    ...
    *,
    ...
    await expect(locator).to_be_visible(timeout=timeout)
    await locator.select_option(value=value, label=label, index=index)
...
async def _is_file_input(locator: Locator) -> bool:
    try:
        return bool(
...
async def _resolve_file_input_target(locator: Locator, page: Optional[Page]) -> Locator:
    base = locator.first
    if await _is_file_input(base):
        return base
    try:
        ...
        pass
    try:
        for_attr = await base.get_attribute("for")
...
async def safe_upload(
    ...
    *,
    ...
    target = await _resolve_file_input_target(locator_or_input, page)
    try:
        await target.wait_for(state="attached", timeout=timeout)
    except Exception:
        pass
    await target.set_input_files(_as_file_list(file_paths))
    if wait_for_completion:
        try:
            await expect(target).to_have_value(re.compile(".+"), timeout=timeout)
        except Exception:
            pass
...
async def dismiss_cookie_banner(page: Page) -> bool:
    ...
    async def _promote_container(candidate):
        try:
            ...
            if await container.is_visible(timeout=600):
                return container
        except Exception:
            pass
        return candidate

    async def _find_cookie_banner():
        for selector in banner_selectors:
            try:
                loc = page.locator(selector)
                try:
                    filtered = loc.filter(has_text=cookie_text).first
                    if await filtered.is_visible(timeout=600):
                        return await _promote_container(filtered)
                except Exception:
                    pass
                candidate = loc.first
                if await candidate.is_visible(timeout=600):
                    return await _promote_container(candidate)
            except Exception:
                continue
        try:
            text_loc = page.get_by_text(cookie_text).first
            if await text_loc.is_visible(timeout=600):
    ...
    async def _try_patterns(scope, patterns) -> bool:
        for pat in patterns:
            try:
    ...
    banner = await _find_cookie_banner()
    if not banner:
        return False

    if await _try_patterns(banner, decline_patterns):
        return True
    if await _try_patterns(banner, accept_patterns):
        return True
    fallback_selectors = [
...
async def safe_expect_dialog(
    ...
    *,
    ...
    async with page.expect_dialog(timeout=timeout) as dlg_info:
        action()
...
async def safe_expect_download(
    page: Page,
    action: Callable[[], None],
    *,
...
async def safe_expect_new_page(
...
async def safe_expect_file_chooser(
    page: Page,
    action: Callable[[], None],
    *,
    ...
    if files is not None:
        await chooser.set_files(_as_file_list(files))
    return chooser


async def safe_expect_response(
    page: Page,
    url_or_predicate,
    *,
    ...
    async with page.expect_response(url_or_predicate, timeout=timeout) as resp_info:
        action()
    return await resp_info.value


async def safe_expect_request(
    page: Page,
    url_or_predicate,
    *,
    action: Optional[Callable[[], None]] = None,
    ...
    async with page.expect_request(url_or_predicate, timeout=timeout) as req_info:
        action()
    return await req_info.value


async def resolve_frame(page: Page, name_or_url: str) -> Optional[Frame]:
    if not name_or_url:
        return None
...
async def safe_key_sequence(page: Page, keys: Sequence[str], delay: int = 50) -> None:
    for key in keys:
        await page.keyboard.press(key, delay=delay)


async def safe_key_chord(page: Page, chord: str, delay: int = 50) -> None:
    await page.keyboard.press(chord, delay=delay)


async def safe_type(page: Page, text: str, delay: int = 0) -> None:
    await page.keyboard.type(str(text), delay=delay)


async def safe_mouse_wheel(page: Page, dx: int = 0, dy: int = 120) -> None:
...
async def _locator_from_candidates(locator: Locator, bbox_hint: Optional[dict] = None) -> Locator:
    try:
...
async def _dispatch_html5_drag(page: Page, source: Locator, target: Locator) -> None:
    source_handle = await source.element_handle()
...
async def safe_drag_and_drop(
    ...
    *,
    ...
    while attempt <= retries:
        attempt += 1
        try:
            src = await _locator_from_candidates(source, source_bbox_hint)
            tgt = await _locator_from_candidates(target, target_bbox_hint)

            await expect(src).to_be_visible(timeout=timeout)
            await expect(tgt).to_be_visible(timeout=timeout)
            try:
                await src.scroll_into_view_if_needed(timeout=timeout)
            except Exception:
                pass
            try:
                await tgt.scroll_into_view_if_needed(timeout=timeout)
            except Exception:
                pass
            ...
            if method in ("auto", "playwright"):
                try:
                    await src.drag_to(tgt, timeout=timeout)
            ...
            if method in ("auto", "mouse"):
                ...
                bbox_s = await src.bounding_box()
                bbox_t = await tgt.bounding_box()
                ...
                await page.mouse.move(*end, steps=max(1, move_steps))
                await page.mouse.up()
                used_method = "mouse"
                if page is not None:
                    page._last_action = f"DRAG_AND_DROP: source -> target (method={used_method})"
            ...
            if method in ("auto", "html5", "js"):
                ...
                await _dispatch_html5_drag(page, src, tgt)
                used_method = "html5"
                if page is not None:
                    page._last_action = f"DRAG_AND_DROP: source -> target (method={used_method})"
                    page._last_locator = src
            ...
            try:
                if page is not None and screenshot_on_fail:
                    await page.screenshot(path=f"drag_drop_retry_{attempt}.png", full_page=False)
            except Exception:
                pass
            continue
